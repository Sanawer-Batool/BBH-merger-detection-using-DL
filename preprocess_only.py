import argparse

import numpy as np
import torch


class SpectralDensity:
    """Compute power spectral density using Welch's method (no ml4gw dependencies)."""

    def __init__(self, sample_rate: float, fftlength: float, overlap: float | None = None):
        self.sample_rate = sample_rate
        self.fftlength = fftlength
        if overlap is None:
            overlap = fftlength / 2
        self.overlap = overlap
        self.nperseg = int(fftlength * sample_rate)
        self.nstride = self.nperseg - int(overlap * sample_rate)
        self.window = np.hanning(self.nperseg)

    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        """Estimate PSD using Welch's method."""
        batch_size, num_channels, num_samples = x.shape
        num_freqs = self.nperseg // 2 + 1
        psd = torch.zeros(batch_size, num_channels, num_freqs, dtype=x.dtype)

        for b in range(batch_size):
            for c in range(num_channels):
                signal = x[b, c].numpy()
                ffts = []
                for start in range(0, len(signal) - self.nperseg + 1, self.nstride):
                    segment = signal[start : start + self.nperseg]
                    windowed = segment * self.window
                    fft_vals = np.fft.rfft(windowed)
                    ffts.append(np.abs(fft_vals) ** 2)

                if ffts:
                    psd_vals = np.mean(ffts, axis=0)
                else:
                    psd_vals = np.ones(num_freqs)

                scale = 1.0 / (self.sample_rate * np.sum(self.window**2))
                psd[b, c] = torch.from_numpy(psd_vals * scale).float()

        return psd


class Whiten:
    """Whiten data using PSD with optional highpass/lowpass filtering."""

    def __init__(
        self,
        fduration: float,
        sample_rate: float,
        highpass: float | None = None,
        lowpass: float | None = None,
    ):
        self.fduration = fduration
        self.sample_rate = sample_rate
        self.highpass = highpass
        self.lowpass = lowpass
        self.pad = int(fduration * sample_rate / 2)

    def __call__(self, x: torch.Tensor, psd: torch.Tensor, crop: bool = True) -> torch.Tensor:
        """Whiten timeseries using the provided PSD."""
        batch_size, num_channels, num_samples = x.shape
        num_freqs = num_samples // 2 + 1

        # Interpolate PSD to match number of frequency bins in x
        if psd.size(-1) != num_freqs:
            psd_interp = torch.nn.functional.interpolate(
                psd, size=(num_freqs,), mode="linear"
            )
        else:
            psd_interp = psd

        whitened = torch.zeros_like(x)

        for b in range(batch_size):
            for c in range(num_channels):
                signal = x[b, c]
                psd_channel = psd_interp[b, c]

                # Remove mean
                signal = signal - signal.mean()

                # FFT
                signal_fft = torch.fft.rfft(signal.double(), norm="forward")

                # Apply highpass/lowpass by zeroing PSD bands
                psd_filtered = psd_channel.clone().double()
                df = self.sample_rate / num_samples
                if self.highpass is not None:
                    idx = int(self.highpass / df)
                    psd_filtered[:idx] = 1e-10  # Near-zero to avoid division issues
                if self.lowpass is not None:
                    idx = int(self.lowpass / df)
                    psd_filtered[idx:] = 1e-10

                # Divide by sqrt(PSD) to whiten
                asd = torch.sqrt(psd_filtered + 1e-10)
                signal_fft = signal_fft / asd

                # IFFT back to time domain
                signal_whitened = torch.fft.irfft(
                    signal_fft, n=num_samples, norm="forward"
                )
                signal_whitened = signal_whitened.float() / (self.sample_rate**0.5)

                whitened[b, c] = signal_whitened

        # Crop edges if requested
        if crop:
            whitened = whitened[:, :, self.pad : -self.pad]

        return whitened


class ChannelWiseScaler:
    """Normalize each channel to zero mean and unit variance."""

    def __init__(self, num_channels: int | None = None):
        self.num_channels = num_channels
        if num_channels is None:
            self.mean = torch.zeros(1)
            self.std = torch.ones(1)
        else:
            self.mean = torch.zeros(num_channels, 1)
            self.std = torch.ones(num_channels, 1)

    def fit(self, x: torch.Tensor, std_reg: float | None = 0.0) -> None:
        """Fit scaling parameters to data."""
        if x.ndim == 1:
            self.mean = x.mean(dim=0, keepdim=True)
            self.std = x.std(dim=0, correction=0, keepdim=True)
        elif x.ndim == 2:
            self.mean = x.mean(dim=-1, keepdim=True)
            self.std = x.std(dim=-1, correction=0, keepdim=True)
        else:
            raise ValueError(f"Expected 1D or 2D tensor, got shape {x.shape}")

        if std_reg is not None:
            self.std = self.std + std_reg

    def __call__(self, x: torch.Tensor, reverse: bool = False) -> torch.Tensor:
        """Apply or reverse scaling."""
        if not reverse:
            return (x - self.mean) / self.std
        else:
            return self.std * x + self.mean


def get_synthetic_strain_data(
    duration: float,
    sample_rate: float,
    batch_size: int,
    num_channels: int,
    seed: int,
) -> torch.Tensor:
    """Create detector-like background strain data as a stand-in for raw input."""
    num_samples = int(duration * sample_rate)
    generator = torch.Generator().manual_seed(seed)

    # Base Gaussian background for each detector channel.
    x = torch.randn(batch_size, num_channels, num_samples, generator=generator)

    # Add a slow drift term to mimic non-stationary behavior.
    t = torch.linspace(0, duration, num_samples)
    drift = 0.05 * torch.sin(2 * torch.pi * 0.5 * t)
    x = x + drift[None, None, :]

    return x.float()


def estimate_psd(
    x: torch.Tensor,
    sample_rate: float,
    fftlength: float,
    overlap: float,
) -> torch.Tensor:
    """Estimate PSD from raw time-series data."""
    spectral_density = SpectralDensity(
        sample_rate=sample_rate,
        fftlength=fftlength,
        overlap=overlap,
    )
    return spectral_density(x)


def apply_preprocessing(
    x: torch.Tensor,
    sample_rate: float,
    fduration: float,
    highpass: float | None,
    lowpass: float | None,
    fftlength: float,
    overlap: float,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Run PSD estimation, whitening, and channel-wise normalization."""
    psd = estimate_psd(x, sample_rate=sample_rate, fftlength=fftlength, overlap=overlap)

    whitener = Whiten(
        fduration=fduration,
        sample_rate=sample_rate,
        highpass=highpass,
        lowpass=lowpass,
    )
    whitened = whitener(x, psd)

    # Fit normalization across batch and time for each channel.
    num_channels = whitened.size(1)
    scaler = ChannelWiseScaler(num_channels=num_channels)
    fit_data = whitened.transpose(0, 1).reshape(num_channels, -1)
    scaler.fit(fit_data, std_reg=1e-8)
    normalized = scaler(whitened)

    return psd, whitened, normalized


def summarize(name: str, x: torch.Tensor) -> None:
    """Print shape and basic statistics for a tensor."""
    print(
        f"{name}: shape={tuple(x.shape)} "
        f"mean={x.mean().item():.4f} std={x.std(unbiased=False).item():.4f}"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Data preprocessing only (ml4gw style)")
    parser.add_argument("--duration", type=float, default=16.0)
    parser.add_argument("--sample-rate", type=float, default=2048.0)
    parser.add_argument("--batch-size", type=int, default=8)
    parser.add_argument("--num-channels", type=int, default=2)
    parser.add_argument("--seed", type=int, default=7)

    parser.add_argument("--fftlength", type=float, default=2.0)
    parser.add_argument("--overlap", type=float, default=1.0)
    parser.add_argument("--fduration", type=float, default=2.0)
    parser.add_argument("--highpass", type=float, default=20.0)
    parser.add_argument("--lowpass", type=float, default=512.0)

    return parser.parse_args()


def main() -> None:
    args = parse_args()

    raw = get_synthetic_strain_data(
        duration=args.duration,
        sample_rate=args.sample_rate,
        batch_size=args.batch_size,
        num_channels=args.num_channels,
        seed=args.seed,
    )

    psd, whitened, normalized = apply_preprocessing(
        raw,
        sample_rate=args.sample_rate,
        fduration=args.fduration,
        highpass=args.highpass,
        lowpass=args.lowpass,
        fftlength=args.fftlength,
        overlap=args.overlap,
    )

    summarize("raw", raw)
    summarize("psd", psd)
    summarize("whitened", whitened)
    summarize("normalized", normalized)


if __name__ == "__main__":
    main()
